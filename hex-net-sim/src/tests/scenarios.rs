...
const SEND_INTERVAL: Span = Span::from_millis(50);
...
const PAST_IDLE_TIMEOUT: Span = Span::from_secs(12);
const PAST_RESUME_GRACE: Span = Span::from_secs(50);

fn link() -> Link {
    Link::clean(Span::from_millis(25))
}
...
fn a_crashed_client_leaves_a_suspended_session_that_later_expires() {
    ...
    world.run_for(Span::from_secs(2)).expect("progress");
...
fn a_rebound_address_keeps_the_connection_and_is_reported() {
    ...
    world.run_for(Span::from_secs(20)).expect("progress");
...
fn a_server_shutdown_ends_every_client() {
    ...
    world.run_for(Span::from_secs(5)).expect("progress");
...
fn a_path_that_degrades_mid_run_still_delivers_everything() {
    let hostile = Link {
        latency: Span::from_millis(90),
        jitter: Span::from_millis(60),
        loss: 0.15,
    ...
    world.run_for(Span::from_secs(60)).expect("progress");
...
fn a_message_lost_in_a_blackout_rides_the_probe_back() {
    let clean = Link::clean(Span::from_millis(50));
    let blackout = Link {
        latency: Span::from_millis(50),
        jitter: Span::ZERO,
        loss: 1.0,
        burst: None,
        duplicate: 0.0,
    ...
    let pace = Pace::steady(Span::from_millis(1500));
    let mut world = World::new(27, 8, ORDERED, Echo::default());
    world.add_client(clean, clean, Chatter::paced(0, 2, pace));

    world.run_for(Span::from_millis(1200)).expect("progress");
    world.set_links(0, blackout, blackout);
    world.run_for(Span::from_millis(800)).expect("progress");
...
fn a_spent_ticket_cannot_connect_again() {
    ...
    world.run_for(Span::from_secs(10)).expect("progress");

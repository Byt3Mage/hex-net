...
/// Maps a float range onto a fixed number of integer steps.
///
/// Arithmetic is done in f64: at nineteen or more bits of steps, f32 lacks the
/// precision to place values consistently near a step boundary.
#[derive(Debug, Clone, Copy)]
...
impl Quantizer {
    pub fn new(min: f32, max: f32, resolution: f32) -> Self {
        assert!((max > min) && (resolution > 0.0));
        let steps = (((max as f64) - (min as f64)) / (resolution as f64)).ceil();
        assert!(steps < (u32::MAX as f64), "range too fine for 32 bits");

        let steps = steps as u32;
        let span = (max as f64) - (min as f64);
        Self {
            min: min as f64,
            scale: (steps as f64) / span,
            inv_scale: span / (steps as f64),
            steps,
        }
    }
    ...
    /// Rounds to the nearest step, so error stays within half a step in either
    /// direction.
    ///
    /// Casting a float to u32 saturates, which maps NaN and out-of-range inputs
    /// to the bounds rather than to arbitrary values.
    #[inline]
    pub fn quantize(&self, value: f32) -> u32 {
        let step = (((value as f64) - self.min) * self.scale).round();
        (step as u32).min(self.steps)
    }

    #[inline]
    pub fn dequantize(&self, step: u32) -> f32 {
        (self.min + ((step as f64) * self.inv_scale)) as f32
    }
}
